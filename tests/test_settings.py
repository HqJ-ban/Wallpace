...
# 将 src/ 加到 sys.path（因为测试可能从不同目录运行）
import sys
...
class TestSettingsValidation:
    ...
    def test_invalid_image_directories(self):
        data = dict(DEFAULT_CONFIG)
        data["image_directories"] = ["/nonexistent/path"]
        errs = Settings.validate(data)
        assert any("无效" in e or "文件夹" in e for e in errs)
    ...
    def test_invalid_daily_time(self):
        data = {**DEFAULT_CONFIG, "switch_mode": "daily_random",
                "daily_time": "25:99"}
        errs = Settings.validate(data)
